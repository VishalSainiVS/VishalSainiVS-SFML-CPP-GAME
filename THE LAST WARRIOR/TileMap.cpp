...
TileMap::TileMap(float gridSize, int width, int height, std::string texture_file)
{
	...
	this->collisionBoxGB.height = gridSize - 20.f;
	this->collisionBoxGB.width = gridSize - 20.f;
	...
	this->collisionBox.setSize(sf::Vector2f(this->collisionBoxGB.height, this->collisionBoxGB.width));
...
const int TileMap::getRewardLeft()
{
	return this->countReward;
}
const int TileMap::getMaxReward()
{
	return this->MaxReward;
}

bool TileMap::gethome()
{
	return this->home;
}

sf::Vector2i TileMap::getHomeGridPos()
{
	return sf::Vector2i(this->homeGridpos);
}
...
void TileMap::addTile(const int x, const int y, const int z, const sf::IntRect& texture_rect, const bool& collision, const short& type)
{
	...
	if (type == 6)
	{
		this->homeGridpos.x = x;
		--this->homeGridpos.x;
		this->homeGridpos.y = y;
	}
	...
	{
		/* OK To add tile. */
		this->map[x][y][z].push_back(new RegularTile(type, x, y, this->gridSizeF, this->tileSheet, texture_rect,   this->collisionBoxGB, collision ));
...
void TileMap::removeTile(const int x, const int y, const int z, const int type)
{
		...
		z < this->layers && z >= 0)
	{
		...
		{
			//std::cout << this->map[x][y][z].back()->getType() << " "<<type <<"\n" ;
			/* OK To remove tile. */
			if (type >= 0)
			{
	...
	*/
	...
	if (out_file.is_open())
	{
		out_file << this->maxSizeWorldGrid.x << " " << this->maxSizeWorldGrid.y << "\n"
			...
			<< this->layers << "\n"
			<< this->textureFile << "\n"
			<< this->homeGridpos.x << " " << this->homeGridpos.y << "\n";
...
void TileMap::loadFromFile(const std::string file_name)
{
	...
	if (in_file.is_open())
	{
		...
		//Basics
		in_file >> size.x >> size.y >> gridSize >> layers >> texture_file >> home_grid_pos.x >> home_grid_pos.y;
		...
		this->layers = layers;
		this->textureFile = texture_file;
		this->homeGridpos = home_grid_pos;
		...
		while (in_file >> x >> y >> z >> type)
		{
			...
			{
				++this->countReward;
				this->MaxReward = this->countReward;
			}
			//std::cout << type << "\n";
			...
			{
				in_file >> trX >> trY >> collision >> collisionboxgb.width >> collisionboxgb.height >>collisionboxgb.top >>collisionboxgb.left ;
	...
	for (int x = this->fromX; x < this->toX; x++)
	{
		for (int y = this->fromY; y < this->toY; y++)
		{
			for (size_t k = 0; k < this->map[x][y][this->layer].size(); k++)
			{
				...
				sf::FloatRect wallBounds = this->map[x][y][this->layer][k]->getCollisionbox();
				sf::FloatRect nextPositionBounds = entity->getNextPositionBounds(dt);
				
				
				//checking REWARD tile 
				if (this->map[x][y][this->layer][k]->getType() == 5)
				{
					sf::FloatRect rewardBounds = this->map[x][y][this->layer][k]->getGlobalBounds();
					if (rewardBounds.intersects(nextPositionBounds))
					{
						if (!entity->getAttributeComponent()->isDead() && !entity->getAttributeComponent()->getRewardStatus())
						{
							if (entity->getAttributeComponent()->getEntityType() == "enemy")
							{
								entity->getAttributeComponent()->setRewardStatus(true);
								this->removeTile(x, y, this->layer, 5);
							}	
						}
						if (entity->getAttributeComponent()->getEntityType() == "player")
						{
							this->removeTile(x, y, this->layer, 5);
							--this->countReward;
						}
				...
				// checking home tile
				else if (this->map[x][y][this->layer][k]->getType() == 6)
				{
					sf::FloatRect homeBounds = this->map[x][y][this->layer][k]->getGlobalBounds();
					if (entity->getAttributeComponent()->getEntityType() == "player" && homeBounds.intersects(nextPositionBounds))
					{
						this->home = true;
					}
				}
				...
				//checking collision
					...
					wallBounds.intersects(nextPositionBounds)
					)
				{
					//Bottom collision
...
void TileMap::update()
{
	this->collisionBox.setSize(sf::Vector2f(this->collisionBoxGB.width, this->collisionBoxGB.height));
}
		...
		for (int y = this->fromY; y < this->toY; y++)
		{
			for (size_t k = 0; k < this->map[x][y][this->layer].size(); k++)
			{
				...
				if (show_collision)
				{
					...
					{
						this->collisionBox.setSize(sf::Vector2f(this->map[x][y][this->layer][k]->getCollisionbox().width, this->map[x][y][this->layer][k]->getCollisionbox().height));
						this->collisionBox.setPosition(sf::Vector2f(this->map[x][y][this->layer][k]->getCollisionbox().left, this->map[x][y][this->layer][k]->getCollisionbox().top));
