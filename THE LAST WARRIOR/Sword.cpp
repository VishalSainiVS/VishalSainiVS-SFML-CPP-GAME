...
Sword::Sword(unsigned range, 
	std::string texture_file)
	: MeleeWeapon( range,  texture_file)
{
	//Visual Weapon
	this->weapon_sprite.setOrigin
	(
		(this->weapon_sprite.getGlobalBounds().width / 2.f),
		this->weapon_sprite.getGlobalBounds().height + 300.f
	);
	this->weapon_sprite.setScale(.13f, .13f);
	this->weapon_sprite.setColor(sf::Color::Transparent);
}
...
void Sword::update(const sf::Vector2f & mouse_pos_view, const sf::Vector2f center)
{
	...
	if (this->attackTimer.getElapsedTime().asMilliseconds() < this->attackTimerMax / 4)
	{
		...
		this->weapon_sprite.setColor(sf::Color(255,255,255,100));
	}
	else 
	{
