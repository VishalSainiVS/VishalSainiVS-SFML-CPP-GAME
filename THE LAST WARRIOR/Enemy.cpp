...
void Enemy::gainHP()
{
	if (this->attributeComponent)
	{
		this->attributeComponent->gainMaxHP();
	}
}

const bool Enemy::isStop() const
{
	if (this->attributeComponent)
	{
		return this->attributeComponent->isStop();
	}
...
void Enemy::setDead(bool is_dead) 
{
	this->attributeComponent->setDead(is_dead);
