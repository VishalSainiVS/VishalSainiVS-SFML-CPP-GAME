...
EnemySpawnerTile::EnemySpawnerTile(int grid_x, int grid_y, float gridSizeF,
	...
	int enemy_type, int enemy_amount, sf::Int32 enemy_time_to_spawn)

	: Tile(TileTypes::ENEMYSPAWNER, grid_x, grid_y, gridSizeF, texture, texture_rect, sf::FloatRect(),false)
{
...
const std::string EnemySpawnerTile::getAsString() const
{
	...
	*/
	...
	ss << this->type << " " << this->shape.getTextureRect().left << " " << this->shape.getTextureRect().top << " "
		<< this->enemyType << " "  << this->enemyAmount << " " << this->enemyTimeToSpawn ;
