...
{
	/*
	 * Converts a scale value to pixels relative to the current resolution in the x-axis.
	 *
...
void gui::Button::update(const sf::Vector2i& mousePosWindow)
{
	...
	{
	case BTN_IDLE:
		if (this->textString == "Off")
		{
			this->idleColor = sf::Color(100, 0, 0, 255);
	...
	case BTN_HOVER:
		if (this->textString == "Editor")
		{
			cursorText.setPosition(mousePosWindow.x + 50.f, mousePosWindow.y - 10.f);
			
			this->cursorText.setString("Only Developer Can Access it !!");
	...
	case BTN_ACTIVE:
		if (this->textString == "On")
		{
			this->idleColor = this->activeColor;
			this->shape.setFillColor(this->activeColor);
			this->text.setFillColor(this->textActiveColor);
			this->shape.setOutlineColor(this->outlineActiveColor);
		...
		else if (this->textString == "Off")
		{
			this->idleColor = sf::Color(0, 100, 0, 255);
			this->shape.setFillColor(this->activeColor);
			this->text.setFillColor(this->textActiveColor);
			this->shape.setOutlineColor(this->outlineActiveColor);
...
void gui::Button::render(sf::RenderTarget& target)
{
	...
	if(this->textString == "Editor" && this->buttonState == BTN_HOVER)
	...
	: keytimeMax(1.4f), keytime(0.f)
{
	...
	this->bounds.setFillColor(sf::Color(0, 70, 70, 200));
...
void gui::TextureSelector::update(const sf::Vector2i& mousePosWindow, const float& dt)
{
	...
	if ((this->hide_btn->isPressed() || sf::Keyboard::isKeyPressed(sf::Keyboard::Key::X)) && this->getKeytime())
	{
		if (this->hidden)
			this->hidden = false;
		else
