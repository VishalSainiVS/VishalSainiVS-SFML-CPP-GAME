...
void Entity::createAttributeComponent( std::string entity_type)
{
	this->attributeComponent = new AttributeComponent( entity_type);
}
...
const sf::Vector2f Entity::getHitboxCenter() const
{
	return this->hitboxComponent->getPosition() +
		sf::Vector2f
		(
...
const float Entity::getSpriteDistance(const Entity& entity, const Entity& entityy) const
{
	return static_cast<float>(sqrt(pow(entityy.getHitboxCenter().x - entity.getSpriteCenter().x, 2) + pow(entityy.getHitboxCenter().y - entity.getSpriteCenter().y, 2)));
}