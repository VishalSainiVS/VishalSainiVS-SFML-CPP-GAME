...
{
	float width = gui::p2pX(2.6f, this->vm);
	float height = gui::p2pY(3.8f, this->vm);
	...
	this->levelBarText.setPosition(
		this->levelBarBack.getPosition().x + gui::p2pX(1.f, this->vm), 
		this->levelBarBack.getPosition().y + gui::p2pY(1.f, this->vm)
	);
}
...
{
	this->levelBarString = std::to_string(this->player->getLevel());
