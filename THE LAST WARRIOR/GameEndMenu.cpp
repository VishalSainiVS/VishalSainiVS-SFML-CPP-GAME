...
GameEndMenu::GameEndMenu(sf::VideoMode& vm, sf::Font& font)
	: font(font)
{
	//Init background
	...
	this->menuText.setPosition(
		...
		this->container.getPosition().y + gui::p2pY(7.f, vm)
	);
}
...
std::map<std::string, gui::Button*>& GameEndMenu::getButtons()
{
	return this->buttons;
}

//Functions
const bool GameEndMenu::isButtonPressed(const std::string key)
{
	return this->buttons[key]->isPressed();
}

void GameEndMenu::addButton(
...
void GameEndMenu::update(const sf::Vector2i& mousePosWindow)
{
...
void GameEndMenu::render(sf::RenderTarget& target)
{
